...
router.put('', verify, async (req, res) => {
    ...
    await Task.findByIdAndUpdate({ _id: req.query._id }, { $set: req.body }, { new: true }, (error, results) => {
        if (error) {
...
router.get('/homeId', verify, async (req, res) => {
    ...
    if (!task.length) return res.status(404).send('No existen tareas para esa casa de salud.');
...
router.get('/user', verify, async (req, res) => {
    ...
    if (!task.length) return res.status(404).send('No existen tareas para ese usuario.');
    else return res.status(200).send(task);
});
